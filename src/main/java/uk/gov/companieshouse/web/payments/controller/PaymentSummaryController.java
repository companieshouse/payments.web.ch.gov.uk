...
public class PaymentSummaryController extends BaseController {
  ...
  public String getPaymentSummary(@PathVariable String paymentId,
    ...
    // Set the available payment methods using the first resource that is to be paid
    // for
    // If there is more than one resource and they have different available payment
    // methods this error will be found in the API.
    ...
    // If there is only one payment method do not display screen to choose payment
    // type
    if (availablePaymentMethods.getAvailablePaymentMethods().size() == 1) {
      if (summary.equals(false)) {
        ...
        // If summary query parameter is set to false do not display summary screen and
        // return ExternalPayment URL
      ...
      if (summary.equals(false)) {
        ...
        LOGGER.errorRequest(request,
            "A parameter cannot be passed to skip the summary screen if there is more than one payment method available");
        return ERROR_VIEW;
      } else {
        // If there is more than one available payment method a screen must be displayed
        // to choose which payment method the user wants to use
  ...
  public String postExternalPayment(
    ...
    // Only validate page if there is more than one payment method available to be
    // chosen
    if (availablePaymentMethods.getAvailablePaymentMethods().size() > 1) {
    ...
    try {
      paymentService.patchPayment(paymentId,
          paymentMethodTransformer.getDataPaymentMethod(paymentMethodChoice.getSelectedPaymentMethod()), isAPIKey);
    } catch (ServiceException e) {
      LOGGER.errorRequest(request, e.getMessage(), e);
      return ERROR_VIEW;
    }
